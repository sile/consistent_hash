...
use clap::{App, Arg};
use consistent_hash::{StaticHashRing, Node, DefaultRingHasher};
...
fn main() {
    ...
    let vnodes = 3000; // TODO: parameter
    let nodes =
        StaticHashRing::new(DefaultRingHasher::new(),
                            matches.values_of("NODES").unwrap().map(|n| Node::new(n, (), vnodes)));
    ...
    let mut counts: HashMap<&str, _> = HashMap::from_iter(nodes.iter().map(|k| (k.key, 0)));
    for word in words.iter() {
